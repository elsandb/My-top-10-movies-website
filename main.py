...
class RateMovieForm(FlaskForm):
    """FlaskForm. Two fields: rating and review. Button: 'Done'."""
...
class AddForm(FlaskForm):
    """FlaskForm. *One field: 'title_query' - to search for a movie title. *One button: 'Search'."""
    title_query = fields.StringField(label='Movie title', validators=[validators.InputRequired()])
    add_button = fields.SubmitField(label='Search')
...
def get_movie_details(movie_id):
    ...
    details = requests.get(url=f"https://api.themoviedb.org/3/movie/{int(movie_id)}", params=params).json()
    return details
...
def update():
    ...
    if up_form.validate_on_submit():  # POST REQUEST
        movie_to_update.rating = up_form.rating.data
        movie_to_update.review = up_form.review.data
        db.session.commit()
